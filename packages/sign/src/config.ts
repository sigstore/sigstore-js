...
export function bundleBuilderFromSigningConfig(
  options: Options
): BundleBuilder {
  ...
  const signer = fulcioSignerFromConfig(
    signingConfig,
    identityProvider,
    fetchOptions
  );
  ...
  switch (bundleType) {
    case 'messageSignature':
      return new MessageSignatureBundleBuilder({ signer, witnesses });
    case 'dsseEnvelope':
      return new DSSEBundleBuilder({ signer, witnesses });
  }
}

function fulcioSignerFromConfig(
  signingConfig: SigningConfig,
  identityProvider: IdentityProvider,
  fetchOptions: FetchOptions
): Signer {
...
function witnessesFromConfig(
  ...
  if (signingConfig.rekorTlogConfig) {
    if (signingConfig.rekorTlogConfig.selector !== ServiceSelector.ANY) {
      throw new Error(
        'Unsupported Rekor TLog selector in signing configuration'
      );
    }
      ...
      new RekorWitness({
        ...
        timeout:
          ...
          tlog.majorApiVersion === 1
            ...
            : Math.min(
                fetchOptions.timeout ||
                  /* istanbul ignore next */ DEFAULT_TIMEOUT,
                DEFAULT_REKORV2_TIMEOUT
              ),
      })
    );
  }
  ...
  if (signingConfig.tsaConfig) {
    if (signingConfig.tsaConfig.selector !== ServiceSelector.ANY) {
...
function certAuthorityService(signingConfig: SigningConfig): Service {
  const compatibleCAs = filterServicesByMaxAPIVersion(
    signingConfig.caUrls,
    MAX_CA_API_VERSION
  );
...
function tlogService(signingConfig: SigningConfig): Service {
  const compatibleTLogs = filterServicesByMaxAPIVersion(
    signingConfig.rekorTlogUrls,
    MAX_TLOG_API_VERSION
  );
...
function tsaService(signingConfig: SigningConfig): Service {
  const compatibleTSAs = filterServicesByMaxAPIVersion(
    signingConfig.tsaUrls,
    MAX_TSA_API_VERSION
  );
...
function sortServicesByStartDate(services: Service[]): Service[] {
  const now = new Date();
...
function filterServicesByMaxAPIVersion(
  services: Service[],
  apiVersion: number
): Service[] {
