...
function createSigningCertHandler(
  ...
  return async (body: string): Promise<HandlerFnResult> => {
    try {
      ...
      const { subject, publicKey } = strict ? parseBody(body) : stubBody();
...
function parseBody(body: string): { subject: string; publicKey: string } {
  const json = JSON.parse(body.toString());
  const oidc = json.credentials.oidcIdentityToken;
  const pem = json.publicKeyRequest.publicKey.content;

  // Decode the JWT
  const { sub } = jose.decodeJwt(oidc);

  /* istanbul ignore next */
  return { subject: sub || DEFAULT_SUBJECT, publicKey: pem };
}
